...
st.set_page_config(page_title="ระบบจัดตารางสอน (Ultimate)", layout="wide")
...
# --- Config ---
DAYS, PERIODS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'], [1, 2, 3, 4, 'Lunch', 5, 6, 7, 8]
VIEWS = {
    'Student': {'lbl': 'นักเรียน', 'id': 'Group_ID', 'cols': ['Room_ID', 'Subject_ID', 'Teacher_Name'], 'leg': ['รหัส', 'ชื่อ', 'ห้อง', 'ครู'], 'leg_c': ['Subject_ID', 'Subject_Name', 'Room_ID', 'Teacher_Name'], 'pfx': 'G-'},
    'Teacher': {'lbl': 'ครูผู้สอน', 'id': 'Teacher_ID', 'cols': ['Room_ID', 'Subject_ID', 'Group_ID'], 'leg': ['รหัส', 'ชื่อ', 'ห้อง', 'นร.'], 'leg_c': ['Subject_ID', 'Subject_Name', 'Room_ID', 'Group_ID'], 'pfx': 'T-'},
    'Room':    {'lbl': 'ห้องเรียน', 'id': 'Room_ID', 'cols': ['Teacher_Name', 'Subject_ID', 'Group_ID'], 'leg': ['รหัส', 'ชื่อ', 'ครู', 'นร.'], 'leg_c': ['Subject_ID', 'Subject_Name', 'Teacher_Name', 'Group_ID'], 'pfx': 'R-'}
}
...
def gen_pdf(df, entities, vkey, t_map):
    ...
    for ent in ([entities] if isinstance(entities, str) else entities):
        ...
        pdf.set_font_size(14); pdf.set_fill_color(240); pdf.cell(20, 8, "Day/Time", 1, 0, 'C', 1)
        for p in PERIODS: pdf.cell(15 if p=='Lunch' else 27, 8, "12:00" if p=='Lunch' else str(p), 1, 0, 'C', 1)
        pdf.ln()
        ...
        for d in DAYS:
            ...
            for p in PERIODS:
                ...
                if p=='Lunch': pdf.set_fill_color(220); pdf.cell(w, 22, "พัก", 1, 0, 'C', 1)
                    ...
                    if not r.empty:
                        info = "\n".join([str(r.iloc[0][c])[:15] for c in cfg['cols']]) # Cut long text
    ...
    with pd.ExcelWriter(out, engine='openpyxl') as writer:
        ...
        for k, cfg in VIEWS.items():
            ...
            for ent in sorted(df[cfg['id']].unique()):
                ...
                piv.fillna('').to_excel(writer, sheet_name=f"{cfg['pfx']}{str(ent)[:20]}".replace(":","-").replace("/","-"))
    return out.getvalue()
...
if up:
    ...
    if len(data) == 4:
        st.sidebar.success("✅ ข้อมูลครบถ้วน")
        ...
        if st.sidebar.button("🚀 สร้างตาราง"):
            with st.spinner("Processing..."):
                ...
                if [i for l in res.values() for i in l]:
                    ...
                    if not una: st.success("100% Success")
                    else: st.warning(f"Unassigned: {len(una)}")
                else: st.error("Failed")
    else: st.sidebar.warning(f"ขาดไฟล์: {set(['Groups','Rooms','Teachers','Subjects']) - data.keys()}")

if 'res' in st.session_state:
    df, t_map = st.session_state.res, st.session_state.t_map
    if st.session_state.una: st.write(st.session_state.una)
    st.divider()
    ...
    if sel:
        ...
        h = "<table style='width:100%; text-align:center; border-collapse:collapse;'><tr style='background:#f0f2f6'><th>Day</th>" + "".join([f"<th>{p}</th>" for p in PERIODS]) + "</tr>"
        for d in DAYS:
    ...
    st.divider(); st.subheader("Download All")
    cols = st.columns(4)
    cols[0].download_button("📥 Excel", gen_excel(df, t_map), "Master.xlsx")
    for i, (k, v) in enumerate(VIEWS.items()):
        lbl = v['lbl'].split('(')[0]
        if cols[i+1].button(f"📄 PDF {lbl}"):
