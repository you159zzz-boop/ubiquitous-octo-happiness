...
def validate(df, key, name):
    ...
    if df.duplicated().sum() > 0:
        ...
        logs.append(f"⚠️ {name}: ลบแถวซ้ำซ้อนออก")
    # 3. Drop duplicate IDs
    if key and key in df:
        dups = df[df.duplicated(subset=key)]
        if not dups.empty:
            df = df.drop_duplicates(subset=key)
            logs.append(f"🔧 {name}: แก้ไขรหัสซ้ำ {len(dups)} รายการอัตโนมัติ")
    return df, logs
...
def load_data(files):
    ...
    if len(d) == 4:
        ...
        if not bad_t.empty:
            ...
            logs.append(f"❌ ลบวิชาทิ้ง {len(bad_t)} รายการ (รหัสครูไม่ถูกต้อง)")
            
        # Check Groups (Updated sub)
        sub = d['Subjects']
        bad_g = sub[~sub['Group_ID'].isin(vg)]
        if not bad_g.empty:
            d['Subjects'] = sub[sub['Group_ID'].isin(vg)]
            logs.append(f"❌ ลบวิชาทิ้ง {len(bad_g)} รายการ (รหัสกลุ่มเรียนไม่ถูกต้อง)")
        ...
        for d in DAYS:
            ...
            for p in PERIODS:
                ...
                if p=='Lunch': pdf.set_fill_color(220); pdf.cell(w, 22, "พัก", 1, 0, 'C', 1)
                    ...
                    if not r.empty:
                        info = "\n".join([str(r.iloc[0][c])[:15] for c in cfg['cols']]) # Truncate text
    ...
    with pd.ExcelWriter(out, engine='openpyxl') as writer:
        ...
        for k, cfg in VIEWS.items():
            ...
            for ent in sorted(df[cfg['id']].unique()):
                ...
                for c in range(2, 11): ws.column_dimensions[chr(64+c)].width = 20
        ...
        if st.sidebar.button("🚀 สร้างตาราง"):
            with st.spinner("กำลังจัดตารางสอน (AI Scheduling)..."):
                ...
                if [i for l in res.values() for i in l]:
                    ...
                    else: st.warning(f"⚠️ จัดไม่ได้ {len(una)} รายการ (โปรดดูรายละเอียดด้านล่าง)")
